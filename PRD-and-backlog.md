...
- **Rationale pre-emption risk (schema)**: `set_trade_rationale` is publicly executable by the anon role (see 2026-09-20 Decision Log entry) so the least-privilege Claude CI step can write rationale text; because the anon key is necessarily public and the field is write-once, anyone who obtains a trade's UUID could call the RPC first and permanently pre-empt the real rationale, with no recovery. Trade IDs are non-enumerable UUIDs, which limits practical exposure — accepted as a real residual risk, not solved in v1.
...
| 2026-09-20 | Monorepo with two independent packages: `app/` (Next.js) and `worker/` (Python via `uv`) share nothing but the Supabase database and `supabase/migrations/`; `alphagate` stays a fully separate repo/PyPI dependency, not folded in | A schema change touches both runtimes at once and should be one atomic PR; per the PRD's architecture the worker (mechanical execution) and app (read-only feed) deliberately share no logic, so a shared code package would add complexity with no payoff | Vercel and Python tooling each need their own subdirectory root configured; the worker can't be released on an independent cadence from the app |
| 2026-09-20 | Hand-written SQL migrations are the schema's single source of truth, applied via the Supabase migration tool, rather than an ORM (e.g. Prisma) owning the schema — both `app/` (TypeScript, generated types) and `worker/` (Python) are plain clients against the same SQL-defined schema | Two language runtimes sharing one schema means letting an ORM in one language own it makes the other language a second-class citizen of that toolchain | No compile-time type safety in Python; TypeScript types are only as fresh as the last manual regeneration — a schema change not followed by regenerating `app/src/lib/database.types.ts` will silently drift |
| 2026-09-20 | `trades.rationale` is nullable and write-once, not required at insert time — "no silent trades" is enforced at the publication boundary (a `feed_trades` view exposes only trades with a non-null rationale) plus a write-once guarantee (the `set_trade_rationale` RPC can only fill a NULL, never overwrite existing text) — instead of making rationale `NOT NULL` at insert (forces an LLM call onto the $0-cost mechanical trade-execution path) or writing a placeholder rationale at insert time (a silent trade with better manners) | Keeps the mechanical trade-execution path free of LLM dependency per PRD section 5, while still guaranteeing nothing reaches the public feed without a real rationale | A trade row can transiently exist in the database without a rationale before the daily Claude step runs; the "every trade has a rationale" invariant now depends on a view + a constraint + (eventually) a CI health check, rather than a single NOT NULL column |
| 2026-09-20 | The `set_trade_rationale` RPC is `SECURITY DEFINER` and publicly executable by the anon role, rather than requiring the service-role key | Preserves least-privilege credential scoping for the narrowly-scoped Claude/prose-writing CI step (which per the 2026-09-19 decision intentionally only holds the anon key), letting it write rationale text without being handed broader database access | Because the anon key is necessarily public (shipped in the Next.js client, usable by anyone), any party who obtains a trade's UUID could call this RPC before the real Claude step does, permanently pre-empting that trade's real rationale — the field is write-once, so there's no recovery once filled. Trade IDs are non-enumerable UUIDs, which limits practical exposure, but this is a real, accepted residual risk, not a theoretical one |
| 2026-09-24 | `alphagate.gate()` owns evaluation — treats models as opaque, runs the caller's metric function on champion and challenger against one shared `Holdout` object, and delegates "better" to a pluggable `Comparator` (default: strict margin comparator, ties go to the incumbent), instead of comparing caller-supplied pre-computed scores directly or requiring a sklearn-style `predict()` interface | Same-holdout scoring and chronology checks become properties of the package itself rather than caller conventions the package can't verify; the pluggable comparator lets the Deflated Sharpe Ratio (backlog item 228) replace naive comparison later without changing the core interface; opaque models keep the package domain-agnostic per the CLAUDE.md rule that alphagate stays free of trading-specific code | Callers must wrap models and data in `Candidate`/`Holdout` types rather than passing plain floats; slightly more API surface than a two-number comparison |
| 2026-09-24 | Chronology is checked inside `gate()` from declared metadata — requires tz-aware datetimes, `trained_through + embargo < holdout.start`, `holdout.end <= as_of`, and validates per-row timestamps if given; violations raise a `LookaheadError` rather than being silently treated as a "reject" decision; non-temporal callers can explicitly opt out via `allow_untimed=True`, which is itself recorded in the log | Makes WolfPack's no-lookahead-bias rule (CLAUDE.md non-negotiable) a hard, checked failure inside the governance package itself, not just a documentation convention callers might violate silently; raising an error (not returning reject) correctly treats time-leakage as a pipeline bug, not a legitimate model-quality judgment; the `embargo` parameter leaves room for stricter Phase 2 purging/embargo validation without an API change | alphagate can only check what the caller declares (e.g. a false `trained_through` timestamp would not be caught) — it cannot detect leakage baked into feature construction upstream; non-temporal users must take an explicit opt-out step |
| 2026-09-24 | A `RecordSink` is a required argument to `gate()` — every decision, promotions and rejections alike, writes a versioned `GateRecord` (reason code, human explanation, both metric scores, comparator parameters/stats, holdout window) before the result is returned to the caller, and a failed write raises rather than silently succeeding | Mirrors the CLAUDE.md "no silent trades" rule at the model-governance layer — no silent promotion/rejection decisions either; logging rejections with reasons from day one means the later backlog item to log rejected challengers (item 235) needs no breaking schema change | Slightly noisier API (callers must pass an explicit `NullSink()` to opt out of logging); a logging outage blocks the promotion decision entirely (deliberately fail-closed); the record schema is committed to early in the project |
| 2026-09-24 | Persona strategies are pure `evaluate(ctx) -> list[TargetPosition]` functions returning a target exposure (long/flat in v1) per ticker, not strategies that place their own orders; a single shared executor diffs each strategy's target against the persona's current position and places whatever paper order is needed to close the gap, instead of strategies placing their own orders directly or adopting a full backtesting/event framework (e.g. backtrader-style `on_bar` callbacks) | Concentrates all order-placement, idempotency, sizing, and paper-safety logic in one place instead of duplicating it across four+ personas; a rule-based strategy (Trend Follower), a model-based strategy (The Analyst), and an ensemble (The Pack) all fit the same "target exposure" shape without forcing "compute an indicator" as the only pattern; missed or repeated cron runs self-correct since the signal is a state ("should be long"), not a one-time event ("a cross happened today"); a full backtesting framework was rejected as unnecessary complexity per the PRD's Complexity Principle (section 5a) for four simple daily long/flat strategies | The very first run will enter positions already mid-trend with no fresh crossover that day (flagged in the signal payload as `crossed_today=false` so the rationale-writing step stays honest about it rather than implying a crossover just fired); trades only happen when a position flips between flat and long, so there's no continuous rebalancing — volatility-targeted position sizing (a separate Week 3 backlog item) will need to revisit this if it wants finer-grained sizing |
| 2026-09-24 | Trend Follower uses an untuned 20-day/50-day simple moving average crossover on daily closes (long when the 20-day average is above the 50-day average, flat otherwise, ties go to flat), applied to a shared 5-ticker universe (SPY, QQQ, AAPL, JPM, XOM) at a fixed $3,000 notional per position, in whole shares only | A slower, more famous pairing (50/200, the "golden cross") fires roughly once a year per ticker — too rare to produce a meaningful feed during a 4-week project; 20/50 is a standard textbook default requiring only ~2.5 months of price history; the parameters are deliberately left untuned against historical data, because tuning would introduce a hidden trial that the Deflated Sharpe Ratio (a later backlog item) would need to correct for, so an off-the-shelf default avoids quietly manufacturing an appearance of edge; the universe is shared across all personas because The Pack needs to blend signals per ticker, and The Scout needs tickers with real news/StockTwits coverage | 20/50 produces more false signals in choppy, range-bound markets than a slower pairing would; whole-share rounding introduces sizing error on higher-priced tickers; changing the shared universe later is a real tradeoff decision that would need its own Decision Log entry, since every persona depends on it; no claim of trading edge is made or implied by this parameter choice |
| 2026-09-24 | Trade idempotency is enforced by a database uniqueness constraint on `(persona_id, ticker, signal_ts)` plus a deterministic Alpaca `client_order_id` (derived from persona slug, ticker, and signal date), rather than keying off `run_id`; each trade's database row is inserted as `status='pending'` before the paper order is submitted to Alpaca, and every run starts by reconciling any still-`pending` rows against Alpaca's actual order state — requires a new migration adding a `client_order_id` column (nullable, unique) and the `(persona_id, ticker, signal_ts)` uniqueness constraint to the `trades` table | `run_id` records which specific run did the work, but keying idempotency on it would let a manual `workflow_dispatch` re-run (which gets a new run ID) place a duplicate trade for a signal already acted on; inserting the database row before submitting the broker order means a crash between the two steps leaves a visible `pending` row to reconcile rather than a paper order that exists at the broker with no corresponding database record (a silent trade, which CLAUDE.md's "no silent trades" rule is meant to prevent even before rationale is attached); a deterministic `client_order_id` makes retried submissions themselves safe, since Alpaca rejects a second order with an ID it's already seen | Requires a schema migration on an already-live table; only one trade per persona/ticker/day is possible by design — a rejected order isn't automatically retried until the next session's signal; the reconciliation step (checking pending orders against Alpaca's actual state at the start of every run) adds real logic and a few more edge cases (partial fills, orders that were never actually submitted, stale unsubmitted orders) that need their own test coverage |
| 2026-09-24 | All personas trade through one shared Alpaca paper account rather than separate paper accounts per persona; each persona's own position is derived from its own filled `trades` rows (buys minus sells), not read back from Alpaca directly; the worker itself is the sole writer of the `prices` table (fetching split-adjusted daily bars from Alpaca before strategies run), and strategies read price history only from that table, never calling Alpaca directly | One set of credentials to manage instead of one per persona; persisting price bars in the database gives a reproducible, shared data source that the Sharpe/DSR calculations, The Analyst's model training, and the frontend can all read from consistently; avoids standing up a separate price-ingestion cron job alongside the daily-trade cron | Each persona's recorded position (from its own trade history) can in principle drift from the shared broker account's actual aggregate position — the design calls for a warn-only sanity check comparing the two rather than hard-enforcing consistency; dividends are not adjusted (only stock splits are), a documented limitation, not a silent gap; relying on Alpaca's free-tier consolidated (SIP) daily bars, which carry a 15-minute reporting delay, means the daily cron schedule needs to move later (21:30 UTC on weekdays instead of the current 21:00 UTC) to stay clear of that delay |

## 9. Backlog

### Week 1 — Foundations
- [x] Set up Alpaca paper trading account, verify API access with a test order
- [x] Scaffold repo: Next.js app, Python worker, Supabase Postgres schema (trades, personas, prices, trust_weights tables)
- [x] Architect `alphagate`'s core promotion logic as a standalone, generic module from day one — champion, challenger, metric fn, holdout data in; promote/reject + log out
- [ ] Confirm Claude Code headless run works end-to-end on one trivial task
- [x] Implement Persona #1 (Trend Follower): strategy logic, places paper trade, logs raw trade to DB
